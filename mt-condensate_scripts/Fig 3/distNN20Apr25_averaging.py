                ...
                if centroidContainer[c].shape[0] == 0:
                    ...
                    for oC in otherChannels:
                        ...
                        if h == 0:
                            ...
                            dist, NN = ([obj[0] for obj in dist], [obj[0] for obj in NN])
                        try:
                            for i in range(len(dist)):  
                                current = objectContainer[oC][i] 
                                compared = objectContainer[c][NN[i]]
                                current.add_dist_to_nn(c, [dist[i], compared])           
                        except TypeError:
                ...
                for key in NNDict.keys():
                    if key.startswith(str(oC) + ' vs ' + str(c)) and not key.endswith('0'):
                        ...
                        for i in range(len(NNDict[key])):
                            place.append(NNDict[key][i])
                            place2.append(distDict[key][i])
                        #except TypeError:
...
def average_intensity_correlation(file_array, obj_array, r: int, channels):
    def normalize(e, max_, min_):
        return (e - min_)/(max_ - min_)

    d = {}
    for i in range(len(file_array)):
        ps = obj_array[i]
        for c in channels:
            sub = [obj for obj in ps if obj.channel == c]
            oc = [temp for temp in channels]
            #oc.remove(c) 
            for k in oc:
                l = []
                for j in sub:
                    ...
                    image = file_array[i].image_trace(j.timestep - 1, j.zstack - 1, k - 1)
                    im = normalize(file_array[i].window(image, x, y, r), 
                                   max(file_array[i].window(image, x, y, r).ravel()), 
                                   min(file_array[i].window(image, x, y, r).ravel()))
                    
                    l.append(im)
                if str(c)+str(k) in d.keys():
                    d[str(c) + ' vs ' + str(k)] = d[str(c)+ ' vs ' +str(k)] + l
                else:
                    d[str(c) + ' vs ' + str(k)] = l
    ...
    for key in d:
        ...
        for i in d[key]:
            if math.isnan(i[0,0]):
                 ...
                 s += i     
        if len(d[key]) != 0:
            a = s/(len(d[key])- countnan)
            avg_intensity_dict[key] = a
        else:
            avg_intensity_dict[key] = np.array([None, None])
            
    return avg_intensity_dict
